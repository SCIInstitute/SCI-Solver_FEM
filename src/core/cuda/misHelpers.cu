...
void misHelpers::aggregateWeightedGraph(int maxSize, int fullSize, int depth, IdxVector_d &adjIndexes, IdxVector_d &adjacency, IdxVector_d &partIn, IdxVector_d &nodeWeights)
{
  int size = adjIndexes.size() - 1;
...
//  DataRecorder::Add("Coarse MIS Count", misCount);
  ...
  while(!complete)
  {
    ...
    if(unallocatedNodes == 0)
    {
      ...
      {
        firstTime = false;
        removeRuntyPartitions(fullSize, partIn, nodeWeights);
        thrust::transform(partIn.begin(), partIn.end(), aggregated.begin(), findAggregated());
        partOut = partIn;
      }
    }
...
void misHelpers::restrictPartitionSize(int maxSize, int fullSize, IdxVector_d &adjIndexes, IdxVector_d &adjacency, IdxVector_d &partition, IdxVector_d &nodeWeights)
{
  ...
  while(largestPart > maxSize)
  {
    ...
    //        // Debugging code:
    //        printf("LargestPart is now %4d\n", largestPart);
    //        if (largestPart < maxSize)
    //        {
    //            IntVector_d checkSizes;
    //            getWeightedPartSizes(partition, nodeWeights, checkSizes);
    //            for (int i = 0; i < checkSizes.size(); i++)
    //            {
    //                int value = partSizes[i], check = checkSizes[i];
    //                if (value != check)
    //                    printf("Mismatch: partSizes[%d] = %d checkSizes[%d] = %d\n", i, value, i, check);
    //            }
    //            printf("Finished checking partSizes\n");
    //            
    //            
    //            IdxVector_h debugOut(partSizes.begin(), partSizes.end());
    //            printf("Part Sizes:\n");
    //            printf("\n %4d: ", 0);
    ...
    // Debugging code:
    //checkPartConnectivity(partSizes.size(), partition, adjIndexes, adjacency, "Checking the connectivity after swapping.");
  }
}
...
bool misHelpers::removeRuntyPartitions(int fullSize, IdxVector_d &partition, IdxVector_d &nodeWeights)
{
  // Getting the part sizes:
...
void misHelpers::checkPartConnectivity(int partCount, IdxVector_d partition, IdxVector_d adjIndexes, IdxVector_d adjacency, char *message)
{
  ...
  if(dummy == 1)
  {
    ...
    //        std::cin >> dummy;
    //
    //        IdxVector_d debugOut = partition;
    //        printf("Partition: \n");
    //        printf("\n %4d: ", 0);
    //        for (int i = 0; i < debugOut.size(); i++)
    //        {
