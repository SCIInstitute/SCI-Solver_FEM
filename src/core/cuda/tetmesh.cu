...
TetMesh *TetMesh::read(const char *nodefilename, const char* elefilename)
{
  ...
  int ne;
  int haslabel;
//	FILE* labelfile = fopen("matrixdata/brain.txt", "r");
  fscanf(elefile, "%d %d %d", &ne, &tmp, &haslabel);
  mesh->tets.resize(ne);
  mesh->matlabels.resize(ne, 0);
//	int l, label1num = 0, label2num = 0;
...
//			if(l == 2) label2num++;
  ...
  else
  {
    for(int i = 0; i < ne; i++)
    {
      fscanf(elefile, "%d %d %d %d %d %d", &tmp, &mesh->tets[i][0], &mesh->tets[i][1], &mesh->tets[i][2], &mesh->tets[i][3], &mesh->matlabels[i]);
    }
  }
//	printf("label1num = %d, label2num = %d. \n", label1num, label2num);
...
//  	FILE* brainele, *brainnode;
//  
//		
//  	brainnode = fopen("brain_jon.node", "w+");
//  	brainele  = fopen("brain_jon.ele", "w+");
//  
...
//    for(int i = 0; i < ne; i++)
//    {
//      fprintf(brainele, "%d %d %d %d %d\n", i, mesh->tets[i][0]-1, mesh->tets[i][1]-1, mesh->tets[i][2]-1, mesh->tets[i][3]-1);
//    }
//
//		fclose(brainnode);
//		fclose(brainele);
