...
TetMesh *TetMesh::read(const char *nodefilename, const char* elefilename) {
   ...
   for (int i = 0; i < nv; i++) {
      if (!nodefile.good()) {
         std::cerr << "Bad Node file, node # "<< i << std::endl;
         exit(0);
      }
      std::getline(nodefile, line);
   ...
   for (int i = 0; i < ne; i++) {
      if (!elefile.good()) {
         std::cerr << "Bad Ele file, ele # " << i << std::endl;
         exit(0);
      }
      std::getline(elefile, line);
      if (line.at(0) == '#' || line.empty()) {
      ...
      if (haslabel == 0) {
         if (sscanf(line.c_str(), "%d %d %d %d %d", &tmp, &mesh->tets[i][0],
                  &mesh->tets[i][1], &mesh->tets[i][2], &mesh->tets[i][3]) != 5) {
            std::cerr << "Bad Ele file, line # " << i << std::endl;
            exit(0);
         }
      } else {
         if (sscanf(line.c_str(), "%d %d %d %d %d %d", &tmp, &mesh->tets[i][0],
                  &mesh->tets[i][1], &mesh->tets[i][2],
                  &mesh->tets[i][3], &mesh->matlabels[i]) != 6) {
            std::cerr << "Bad Ele file, line # " << i << std::endl;
            exit(0);
         }
      }
