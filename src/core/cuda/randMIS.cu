...
void RandMIS_Aggregator<Matrix, Vector>::computePermutation(int nn, int* xadj, int* adjncy, IdxVector_h &permutation, IdxVector_h &ipermutation, IdxVector_h &aggregateIdx, IdxVector_h &partitionIdx, int* partitionlabel, int* nnout, int* &xadjout, int* &adjncyout, int metissize)
{
  ...
  // Running a check on the new adjacency structure:
//  printf("Beginning consistency check for initial induced adjacency\n");
//  for(int aIt = 0; aIt < finePartCount; aIt++)
//  {
//    for(int nIt = newAdjacencyIndexes[aIt]; nIt < newAdjacencyIndexes[aIt + 1]; nIt++)
//    {
//      int neighbor = newAdjacency[nIt];
//      bool matched = false;
//      for(int j = newAdjacencyIndexes[neighbor]; j < newAdjacencyIndexes[neighbor + 1]; j++)
//      {
//        if(newAdjacency[j] == aIt)
//          matched = true;
//      }
//      if(!matched)
//      {
//        printf("Mismatch found! %d has neighbor %d but not reverse.\n", aIt, neighbor);
//      }
//    }
//  }
//  printf("Finished checking initial adjacency\n");
...
//  // Running a check on the new adjacency structure:
//  printf("Checking final induced adjacency\n");
//  for(int aIt = 0; aIt < finePartCount; aIt++)
//  {
//    for(int nIt = aggregateAdjacencyOffsets[aIt]; nIt < aggregateAdjacencyOffsets[aIt + 1]; nIt++)
//    {
//      int neighbor = permutedBlockAdjacency[nIt];
//      bool matched = false;
//      for(int j = aggregateAdjacencyOffsets[neighbor]; j < aggregateAdjacencyOffsets[neighbor + 1]; j++)
//      {
//        if(permutedBlockAdjacency[j] == aIt)
//          matched = true;
//      }
//      if(!matched)
//      {
...
//    }
//  }
//  printf("Finished checking final induced adjacency\n");
  ...
  *nnout = finePartCount;
  ...
  // Checking that the finalPermutation is consistent with newAdjacency
  //    printf("Beginning check of finalPermutation\n");
  //    for (int vIt = 0; vIt < nn; vIt++)
  //    {
  //        // Get the new index of the node
  //        int permIdx = permutation[vIt];
  //        
  //        // Find the aggregate for this node:
  //        int currentAgg = -1;
  //        for (int aIt = 0; aIt < finePartCount; aIt++)
  //        {
  //            if(partitionIdx[aIt] <= permIdx && partitionIdx[aIt + 1] > permIdx)
  //            {
  //                currentAgg = aIt;
  //                break;
  //            }
  //        }
  //        
  //        // Find all neighbors of the node in original adjacency
  //        for (int nIt = xadj[vIt]; nIt < xadj[vIt+1]; nIt++ )
  //        {
  //            int permNeighbor = permutation[adjncy[nIt]];
  //
  //            // Find the aggregate for this neighbor:
  //            int neighborAgg = -1;
  //            for (int aIt = 0; aIt < finePartCount; aIt++)
  //            {
  //                if(partitionIdx[aIt] <= permIdx && partitionIdx[aIt + 1] > permIdx)
  ...
  //            }
  //            
  //            // Checking to see if the aggregates are connected in the induced graph
  //            if (neighborAgg != currentAgg)
  //            {
  //                if (neighborAgg == -1 || currentAgg == -1)
  //                {
  //                    printf("Error finding aggregate for %d or %d in original graph!\n", adjncy[nIt], vIt);
  //                }
  //                else
  //                {
  //                    bool found = false;
  //                    for(int anIt = xadjout[currentAgg]; anIt < xadjout[currentAgg + 1]; anIt++)
  //                    {
  //                        if (adjncyout[anIt] == neighborAgg)
  //                            found = true;
  //                    }
  //                    if(!found)
  //                    {
  //                        printf("Vertices %d and %d in original graphs are neighbors, but their aggregates are not!\n", vIt, adjncy[nIt]);
  //                    }
  //                }
  //            }
  //        }
  //        
  //    }
  //    printf("Check of finalPermutation complete\n");

//    if(aggregateIdx[aggregateIdx.size() - 1] != nn)
//    {
//    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
//    printf("The last entry in aggregateIdx is: %d not: %d which is nn\n", aggregateIdx[aggregateIdx.size() - 1], nn);
//    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
//    }
...
template <class Matrix, class Vector>
void RandMIS_Aggregator<Matrix, Vector>::computePermutation_d(TriMesh *meshPtr, IdxVector_d &permutation, IdxVector_d &ipermutation, IdxVector_d &aggregateIdx, IdxVector_d &partitionIdx, IdxVector_d &partitionLabel, IdxVector_d &adjIndexesOut, IdxVector_d &adjacencyOut, int parameters, int part_max_size)
{
    IdxVector_d adjIndexesIn, adjacencyIn;
    misHelpers::getAdjacency(meshPtr, adjIndexesIn, adjacencyIn);
    computePermutation_d(adjIndexesIn, adjacencyIn, permutation, ipermutation, aggregateIdx, partitionIdx, partitionLabel, adjIndexesOut, adjacencyOut, parameters, part_max_size);   
}

template <class Matrix, class Vector>
void RandMIS_Aggregator<Matrix, Vector>::computePermutation_d(TetMesh *meshPtr, IdxVector_d &permutation, IdxVector_d &ipermutation, IdxVector_d &aggregateIdx, IdxVector_d &partitionIdx, IdxVector_d &partitionLabel, IdxVector_d &adjIndexesOut, IdxVector_d &adjacencyOut, int parameters, int part_max_size)
{
    IdxVector_d adjIndexesIn, adjacencyIn;
    misHelpers::getAdjacency(meshPtr, adjIndexesIn, adjacencyIn);
    computePermutation_d(adjIndexesIn, adjacencyIn, permutation, ipermutation, aggregateIdx, partitionIdx, partitionLabel, adjIndexesOut, adjacencyOut, parameters, part_max_size);   
...
template <class Matrix, class Vector>
void RandMIS_Aggregator<Matrix, Vector>::extendedMIS(int n, int partSize, int *adjIndexes, int *adjacency, int *partition, int *partCount)
{
...
//  srand(time(NULL));
  ...
  // For testing set root nodes into a partition and other nodes in another
  //    for (int i = 0; i < n; i++)
  //    {
  //        partition[i] = MIS[i];
  //    }
    ...
    if(!incomplete)
    {
      ...
      if (partsToRemove.size() != partSizes.size())
      {
          ...
          for(int i = partsToRemove.size() - 1; i > -1; i--)
          {
            ...
            //for(int j = partsToRemove[i]; j < partSizes.size() - 1; j++)
            //    partSizes[j] = partSizes[j + 1];
            //partSizes.resize(partSizes.size()-1);
            *partCount = *partCount - 1;
            partSizes.erase(partSizes.begin() + partsToRemove[i]);
          }
          printf("Removed %d undersized aggregates out of %d total. Leaving %d\n", partsToRemove.size(), originalPartCount, *partCount);
      }
  }
