 ...
 *********************************************************/
template <class Matrix, class Vector>
void AMG<Matrix, Vector>::setup(const Matrix_d &Acsr_d, TriMesh* meshPtr, TetMesh* tetmeshPtr)
{
...
//	Ahyb_d_CG = level->A_d;
  ...
  while(true)
  {
    ...
    //resize vectors
//    level->xc_d.resize(nextN, -1);
//    level->bc_d.resize(nextN, -1);
...
void AMG<Matrix, Vector>::solve_iteration(const Vector_d_CG &b, Vector_d_CG &x)
{
  switch(solver)
  {
    case AMG_SOLVER:
      ...
      //fine->cycle(cycle, b, x);
      break;
    case PCG_SOLVER:
      //create a single CG cycle (this will run CG immediatly)
      CG_Flex_Cycle<Matrix_h_CG, Vector_h_CG > (cycle, cycle_iters, fine, Ahyb_d_CG, b, x, tolerance, max_iters); //DHL
      break;
  }
}
...
void AMG<Matrix, Vector>::solve(const Vector_d_CG &b_d, Vector_d_CG &x_d)
{
  ...
  do
  {
    ...
    //    computeResidual(fine->A_d, x_d, b_d, r_d);
    ...
    //    iter_stop = CLOCK();
    //    std::cout << std::setw(15) << iterations << std::setw(15) << iter_stop - iter_start << std::scientific << std::setw(15) << nrm << std::setw(15) << std::fixed;
    //    if(last_nrm == -1) std::cout << std::endl;
    //    else std::cout << std::setprecision(4) << nrm / last_nrm << std::setprecision(6) << std::endl;
...
void AMG<Matrix, Vector>::printGridStatistics()
{
  ...
  std::cout << "     Operator Complexity: " << total_nnz / (double)fine->A_d.num_entries << std::endl;
	...
	/*
	 * Removed DataRecorder
	 *
  //Recording info
  DataRecorder::Add("AMG Grid Complexity", total_rows / (double)fine->A_d.num_rows);
  DataRecorder::Add("AMG Operator Complexity", total_nnz / (double)fine->A_d.num_entries);
  */
}
