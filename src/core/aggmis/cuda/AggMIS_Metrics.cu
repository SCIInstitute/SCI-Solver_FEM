...
namespace AggMIS {
    namespace Metrics {
        MetricsContext::MetricsContext(Graph_h& graph, IntVector_h& aggregation) {
            this->graph = &graph;
...
//                cin >> s;
        ...
        //        printf("Checking around node %d with score %d\n", currentNode, scores[currentNode]);
                betterFound = false;
                int start = graph->indices->data()[currentNode];
                int end = graph->indices->data()[currentNode + 1];
                for (int i = start; i < end && !betterFound; i++)
                {
                    int neighbor = graph->adjacency->data()[i];
        ...
        map<int,int>* MetricsContext::FindDistances(int rootNode, vector<int>& aggregate) {
            ...
            {   
                ...
                for (int j = graph->indices->data()[node]; j < graph->indices->data()[node + 1]; j++)
                {
                    int neighbor = graph->adjacency->data()[j];
        ...
        vector<vector<int> >* MetricsContext::GetShortestPaths(int startId, int endId, map<int,int> &distances) {     
            ...
            while (activePath < paths.size())
            {
                ...
                for (int nIt = graph->indices->data()[endNode]; nIt < graph->indices->data()[endNode + 1]; nIt++)
                {
        ...
        set<int>* MetricsContext::BruteForceMinimalNodes(vector<vector<vector<int> > >& pathOptions) {
            ...
            if (possibilities > 100)
            {
                ...
                Display::Print(pathOptions, "Options");
