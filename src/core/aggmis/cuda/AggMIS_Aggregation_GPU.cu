...
namespace AggMIS {
    namespace Aggregation {
        ...
        // Functions
        IntVector_d* AggregateToNearest(Graph_d &graph, 
                                IntVector_d &roots) {
            // Create temp vectors to work with
            int size = graph.Size();
            IntVector_d *aggregated = new IntVector_d(roots);
            IntVector_d *partIn = new IntVector_d(roots);
            ...
            IntVector_d *partOut = new IntVector_d(*partIn);
        ...
        bool IsValidAggregation(Graph_d &graph, 
            ...
            IntVector_d scratch(aggregation);
            ...
            IntVector_d *valuesIn = GraphHelpers::GetIndicesVector(aggregation.size());
            IntVector_d valuesOut(aggregation.size());
            ...
            if (newLength != correctLength)
            {
                ...
                IntVector_h aggIds(*valuesIn);
                IntVector_h nodeIds(valuesOut);
        ...
        Graph_d* GetAggregateMap(IntVector_d& aggregation) {
            Graph_d* output = new Graph_d();
            // Setting adjacency of output to be indices 
            GraphHelpers::SetToIndicesVector(aggregation.size(), *(output->adjacency));
            IntVector_d aggLabels(aggregation.begin(), aggregation.end());
