...
namespace AggMIS {
  namespace GraphHelpers {
    ...
    AggMIS::Types::Graph_d* GetInducedGraph(AggMIS::Types::Graph_d &graph,
      AggMIS::Types::IntVector_d &aggregation) {
      // Get references to parts of input graph and output graph
      AggMIS::Types::IntVector_d &adjIndexesIn = *(graph.indices);
      AggMIS::Types::IntVector_d &adjacencyIn = *(graph.adjacency);
      AggMIS::Types::Graph_d *result = new AggMIS::Types::Graph_d();
      AggMIS::Types::IntVector_d &adjIndexesOut = *(result->indices);
      AggMIS::Types::IntVector_d &adjacencyOut = *(result->adjacency);

      // Declaring temporary vectors:
      AggMIS::Types::IntVector_d adjacencyBlockLabel, blockMappedAdjacency;
    ...
    AggMIS::Types::Graph_h* GetInducedGraph(AggMIS::Types::Graph_h& graph,
      AggMIS::Types::IntVector_h& aggregation) {
      // Get references to graph indices and adjacency
      AggMIS::Types::IntVector_h &ind = *(graph.indices);
      AggMIS::Types::IntVector_h &adj = *(graph.adjacency);
      ...
      AggMIS::Types::Graph_h *result = new AggMIS::Types::Graph_h();
      AggMIS::Types::IntVector_h &indOut = *(result->indices);
      AggMIS::Types::IntVector_h &adjOut = *(result->adjacency);
    ...
    void mapAdjacencyToBlock(AggMIS::Types::IntVector_d &adjIndexes,
      AggMIS::Types::IntVector_d &adjacency,
      AggMIS::Types::IntVector_d &adjacencyBlockLabel,
      AggMIS::Types::IntVector_d &blockMappedAdjacency,
    ...
    void getPartIndicesNegStart(AggMIS::Types::IntVector_d& sortedPartition,
      AggMIS::Types::IntVector_d& partIndices) {
      // Sizing the array:
    ...
    AggMIS::Types::IntVector_d* GetIndicesVector(int size) {
      thrust::counting_iterator<int> start(0);
      thrust::counting_iterator<int> end = start + size;
      return new AggMIS::Types::IntVector_d(start, end);
    }
    void SetToIndicesVector(int size,
      AggMIS::Types::IntVector_d& toSet) {
      thrust::counting_iterator<int> start(0);
      thrust::counting_iterator<int> end = start + size;
      toSet.assign(start, end);
    }
    void getPartSizes(AggMIS::Types::IntVector_d &aggregation,
      AggMIS::Types::IntVector_d &sizes) {
      // Make a copy of the partition vector to mess with:
      AggMIS::Types::IntVector_d temp(aggregation);
      ...
      AggMIS::Types::IntVector_d partIndices(maxPart + 2, 0);
    ...
    void getPartSizes(AggMIS::Types::IntVector_d& aggregation,
      AggMIS::Types::IntVector_d& sizes,
      AggMIS::Types::IntVector_d& weights) {
      // Make copies to mess with
      AggMIS::Types::IntVector_d tempAgg(aggregation.begin(), aggregation.end());
      AggMIS::Types::IntVector_d tempWeight(weights.begin(), weights.end());
      ...
      AggMIS::Types::IntVector_d accumulatedSize(tempAgg[tempAgg.size() - 1] + 1);
      ...
      Kernels::accumulatedPartSizesKernel << < nBlocks, blockSize >> >
        (size,
        AggMIS::Types::StartOf(tempAgg),
        AggMIS::Types::StartOf(tempWeight),
        AggMIS::Types::StartOf(accumulatedSize));
    ...
    AggMIS::Types::IntVector_d* GetValences(AggMIS::Types::Graph_d &graph) {
      AggMIS::Types::IntVector_d *result = new AggMIS::Types::IntVector_d(graph.indices->size() - 1);
      thrust::adjacent_difference(graph.indices->begin() + 1,
        graph.indices->end(),
        result->begin());
      return result;
    }
    AggMIS::Types::IntVector_h* GetValences(AggMIS::Types::Graph_h &graph) {
      AggMIS::Types::IntVector_h *result = new AggMIS::Types::IntVector_h(graph.indices->size() - 1);
      thrust::adjacent_difference(graph.indices->begin() + 1,
        graph.indices->end(),
        result->begin());
      return result;
    }
    bool IsGraphValid(AggMIS::Types::Graph_d& graph) {
      // Call the override with the indices and adjacency of the graph
      return IsGraphValid(*graph.indices, *graph.adjacency);
    }
    bool IsGraphValid(AggMIS::Types::Graph_h& graph) {
      // Call the override with the indices and adjacency of the graph
      return IsGraphValid(*graph.indices, *graph.adjacency);
    }
    bool IsGraphValid(AggMIS::Types::IntVector_d& indices, AggMIS::Types::IntVector_d& adjacency) {
      // Get temporary host vectors to call with
      AggMIS::Types::IntVector_h tempInd(indices);
      AggMIS::Types::IntVector_h tempAdj(adjacency);
    ...
    bool IsGraphValid(AggMIS::Types::IntVector_h& indices, AggMIS::Types::IntVector_h& adjacency) {
      // Get size of graph
