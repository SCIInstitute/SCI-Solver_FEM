...
namespace AggMIS {
  namespace MergeSplitCPU {
    MergeSplitConditionerCPU::MergeSplitConditionerCPU(AggMIS::Types::Graph_h &graph,
      ...
      AggMIS::Types::IntVector_h* ps = Aggregation::GetPartSizes(aggregation);
      partSizes.swap(*ps);
      delete ps;
    ...
    void MergeSplitConditionerCPU::SetNodeWeights(AggMIS::Types::IntVector_h &input) {
      nodeWeights.swap(input);
      AggMIS::Types::IntVector_h *ws = Aggregation::GetPartSizes(aggregation, nodeWeights);
      weightedSizes.swap(*ws);
      ws->clear();
      delete ws;
    }
    AggMIS::Types::IntVector_h* MergeSplitConditionerCPU::GetAggregation() {
      return &aggregation;
    }
    AggMIS::Types::IntVector_h* MergeSplitConditionerCPU::GetNodeWeights() {
      return &nodeWeights;
    }
    ...
    void MergeSplitConditionerCPU::CycleMergeSplits(float minImprove,
      ...
      AggMIS::Types::IntVector_h *sizes = &partSizes;
    ...
    bool MergeSplitConditionerCPU::Condition(int desiredSize,
      ...
      int maxCycles) {
      if (verbose)
        PrintProgress(&std::cout, "Starting conditioning.", true, true, true);
      ...
      while (currentRatio > tolerance && counter++ < maxCycles)
      {
        ...
        if (verbose)
        {
          ...
          PrintProgress(&std::cout, ss.str(), true, true, true);
        }
      }
      ...
      if (verbose)
        PrintProgress(&std::cout, "After conditioning completed.", true, true, true);
    ...
    void MergeSplitConditionerCPU::PrintProgress(std::ostream* output,
      std::string note,
      bool graphStat,
      bool progressStat,
      bool sizeStat) {
    ...
    void MergeSplitConditionerCPU::PrintSizeStats(std::ostream* output,
      bool makeHeader) {
      if (makeHeader)
        *output << "\n--------------------- Size Check --------------------\n";
    ...
    void MergeSplitConditionerCPU::PrintProgressStats(std::ostream* output,
      bool makeHeader) {
      if (makeHeader)
        *output << "\n------------------- Progress Check ------------------\n";
    ...
    void MergeSplitConditionerCPU::PrintGraphStats(std::ostream* output,
      bool makeHeader) {
      if (makeHeader)
        *output << "\n----------------- Graph Information -----------------\n";
      ...
      AggMIS::Types::IntVector_h *valences = GraphHelpers::GetValences(*graph);
    ...
    void MergeSplitConditionerCPU::InteractiveConsole(std::string message) {
      // Start off by printing overall status info and message
      PrintProgress(&std::cout, message, true, true, true);
      ...
      while (operation != 'd')
      {
        if (operation == 'o' || operation == 'f')
        {
          ...
          if (operation == 'm')
          {
            ...
            PrintProgress(&std::cout, msg, false, true, true);
          }
          if (operation == 's')
          {
            //                        if (cycling)
          ...
          if (operation == 'g')
          {
            ...
            //                            MakeMergeSplits(desiredSize);
            MakeMergeSplits(desiredSize);
            PrintProgress(&std::cout, "After merge-splits", false, true, true);
          }
        } else if (operation == 's') {
        ...
        } else if (operation == 'c')
        {
          ...
          PrintProgress(&std::cout, "After conditioning", false, true, true);
        } else if (operation == 'v') {
    ...
    bool MergeSplitConditionerCPU::MarkMerges(bool force) {
      ...
      AggMIS::Types::IntVector_h &sizes = nodeWeights.size() > 0 ? weightedSizes : partSizes;
    ...
    int MergeSplitConditionerCPU::MergeAggregates(int aggA, int aggB, bool fillSpot) {
      ...
      // Removing edges to aggB and replastd::cing with edges to aggA
      ...
      std::vector<int> temp(aggAdjacency[aggA].size() +
    ...
    void MergeSplitConditionerCPU::SplitAggregate(int agg, int newAgg) {
      if (verbose) {
        ...
        AggMIS::Types::Display::Print(aggMap[agg], s1.str());
      }
      ...
      std::vector<std::vector<int> > *am = Aggregation::GetAggregateGraph(*graph, aggMap[agg]);
      std::vector<std::vector<int> > &aggGraph = *am;

      // Getting the node weights if needed
      AggMIS::Types::IntVector_h weights;
      if (nodeWeights.size() > 0) {
      ...
      std::vector<int> allocated(aggGraph.size(), -1);
      ...
      std::vector<std::queue<int> > filler(2);
      filler[0].push(rootA);
      filler[1].push(rootB);

      // Nodelists for each aggregate
      std::vector<std::vector<int> > nodeLists(2);

      // Sizes of each aggregate
      std::vector<int> aggSizes(2, 0);
      ...
      if (verbose) {
        std::stringstream s2;
        s2 << "AggMap for " << agg;
        AggMIS::Types::Display::Print(aggMap[agg], s2.str());

        std::stringstream s3;
        s3 << "AggMap for " << newAgg;
        AggMIS::Types::Display::Print(aggMap[newAgg], s3.str());
      }
    ...
    void MergeSplitConditionerCPU::LinkAggregate(int aggId) {
      ...
      std::vector<int>::iterator newEnd = unique(aggAdjacency[aggId].begin(),
    ...
    void MergeSplitConditionerCPU::ValidateAggAdjacency() {
      ...
      std::vector<std::vector<int> > temp(aggAdjacency.size());
      ...
      // Compare
      if (AggMIS::Types::Compare::AreEqual(temp, aggAdjacency, true))
    ...
    void MergeSplitConditionerCPU::ValidateAggMap() {
      // Move current into new spot
      std::vector<std::vector<int> > temp(aggMap.size());
      for (int i = 0; i < temp.size(); i++)
      ...
      // Compare
      if (AggMIS::Types::Compare::AreEqual(temp, aggMap, true))
    ...
    void MergeSplitConditionerCPU::ValidatePartSizes() {
      bool failed = false;
      AggMIS::Types::IntVector_h *ps = Aggregation::GetPartSizes(aggregation);
      if (AggMIS::Types::Compare::AreEqual(*ps, partSizes, true))
      ...
      if (nodeWeights.size() > 0) {
        AggMIS::Types::IntVector_h* ws = Aggregation::GetPartSizes(aggregation, nodeWeights);
        if (AggMIS::Types::Compare::AreEqual(*ws, weightedSizes, true))
    ...
    void MergeSplitConditionerCPU::ValidateArraySizes(std::string message) {
