    ...
    namespace Aggregation {
        ...
        namespace Kernels {
            ...
            __global__ void checkAggregationFillAggregates(int size, 
                                            ...
                                            int* incomplete);
            /**
             * This kernel has each node look at its neighbors and mark whether 
             * the edge connecting them crosses an aggregate boundary.
             * @param size Number of nodes in graph
             * @param adjIndices Graph adjacency indices
             * @param adjacency Graph adjacency
             * @param aggregation Current aggregation
             * @param edgesCut vector with cut edges marked as 1 and uncut as 0
             */
            __global__ void markEdgesCut(int size,
                                            int* adjIndices,
                                            int* adjacency,
                                            int* aggregation,
                                            int* edgesCut);
        ...
        static double GetEdgeCutRatio(Graph_d &graph,
                                IntVector_d& aggregation) { return 0; }
    }
}
