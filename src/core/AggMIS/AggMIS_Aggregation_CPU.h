...
namespace AggMIS {
    namespace Aggregation {
        ...
        int FindMassScore(vector<vector<int> > &graph,
                                int startPoint);
        /**
         * This method finds the node(s) for which the massScore is minimal in
         * the given graph.
         * @param graph The graph to find centroidal nodes in
         * @param startPoint The node to start searching for the centroid at
         * @return The set of nodes which are centroidal.
         */
        vector<int>* GetCentroid(vector<vector<int> > &graph, 
                                int startPoint);
        void SubgraphDetails(vector<vector<int> >& subgraph);
    }
}
